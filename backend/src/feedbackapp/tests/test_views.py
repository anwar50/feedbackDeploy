...
class UpdateTeachersTestCase(TestCase):
    '''Test view for updating teacher profiles '''
    def setUp(self):
        user = User(username="John")   
        user.save()
        self.john = TeacherProfile.objects.create(
            user = user,
            department = 'EECS',
            user_type = 'Teacher',
            avatar = 'https://img.icons8.com/color/96/000000/user.png',
        ...
        self.invalid_teacher = {
            ...
            'department': "EECS",
            'user_type': 'Teacher',
            'avatar': 'https://img.icons8.com/color/26/000000/user.png'
        }
    ##  update data!!  ##
    def test_valid_update_teacher(self):
        response = client.put(
            reverse('updateprofile', kwargs={'pk': self.john.pk}),
            data = json.dumps(self.valid_teacher.__dict__),
            content_type = 'application/json'
        )
